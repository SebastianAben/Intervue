...
Use a unique `POSTGRES_PASSWORD` per environment. `DATABASE_URL` is generated by Docker Compose from `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB`, so it does not need to be entered manually.

Keep `POSTGRES_PASSWORD` URL-safe, for example letters, numbers, `_`, `-`, and `.`, because it is embedded into the PostgreSQL connection URL.
